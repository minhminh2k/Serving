...
# OUTPUT_DIR = "sdxl/"
OUTPUT_DIR = "/mnt/nvme1n1/duong.quang.minh/sdxl"
...
def convert_models(
    ...
    logging.info(f"Converting models to ONNX format with opset {opset} and fp16={fp16}")
    ...
    logging.info(f"Text encoder input shape: {text_input.input_ids.to(device=device, dtype=torch.int32).shape}")
    ...
    print(f"VAE Encoder in_channels: {vae_in_channels}, sample_size: {vae_sample_size}")
    ...
    print(f"UNet in_channels: {unet_in_channels}, sample_size: {unet_sample_size}, cross_attention_dim: {cross_attention_dim}, text_hidden_size: {text_hidden_size}")
    
    vae_decoder = pipeline.vae
    vae_latent_channels = vae_decoder.config.latent_channels
    vae_out_channels = vae_decoder.config.out_channels
    print(f"VAE Decoder latent_channels: {vae_latent_channels}, out_channels: {vae_out_channels}")
    ...
    if image_encoder is not None:
        ...
        print(f"Elements Text Encoder 2: {elements}") # [['input_ids']]
        print(f"ONNX outputs Text Encoder 2: {onnx_outputs}") # [['text_embeds', 'last_hidden_state', 'hidden_states']]
        
        end_time = time.time()
        print(f"Export Text encoder 2 completed in {end_time - start_time:.2f} seconds.")

        # validate_models_outputs(
        #     models_and_onnx_configs={"text_encoder_2": (text_encoder_2, CLIPTextModelWithProjectionOnnxConfig(text_encoder_2.config))},
        #     onnx_named_outputs=[['text_embeds', 'last_hidden_state', 'hidden_states']],
        #     output_dir=Path(OUTPUT_DIR),
        #     onnx_files_subpaths=["text_encoder_2.onnx"],
        #     device=device,
        # )
