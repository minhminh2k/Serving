...
class TritonPythonModel:
        ...
        for request in requests:
            ...
            if do_classifier_free_guidance and zero_out_negative_prompt:
                ...
                if do_classifier_free_guidance:
                    ...
                    negative_prompt_embeds = negative_prompt_embeds.to(dtype=torch.float32, device=self.device)
            ...
            for i, t in enumerate(timesteps):
                ...
                logging.error(f"Latent model input shape: {latent_model_input.shape}")
                
                t = t.expand(latent_model_input.shape[0])
                
                logging.error(f"Timestep shape: {t.shape}, Timestep: {t}")
                
                inputs = [
                    pb_utils.Tensor.from_dlpack(
                        "sample", torch.to_dlpack(latent_model_input.to(torch.float32))
                    ),
                    pb_utils.Tensor.from_dlpack(
                        "timestep", torch.to_dlpack(t.to(torch.float32))),
                    pb_utils.Tensor.from_dlpack(
                        "encoder_hidden_states", torch.to_dlpack(prompt_embeds.to(torch.float32))
                    ),
                    pb_utils.Tensor.from_dlpack(
                        "text_embeds", torch.to_dlpack(add_text_embeds.to(torch.float32))
                    ),
                    pb_utils.Tensor.from_dlpack(
                        "time_ids", torch.to_dlpack(add_time_ids.to(torch.float32))
                    ),
                ]
                ...
                logging.error(f"Noise prediction shape: {noise_pred.shape}, Noise prediction: {noise_pred.max()}, {noise_pred.min()}")
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        ...
        if self.vae_force_upcast:
            image = image.float().to(torch.float16)
